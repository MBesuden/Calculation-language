 ...
 * 	- fixed references to FixedPointInfo
 */
...
#include <stdint.h>
#if defined (__arm__)
#define int32_t	int	//workaround for cross compilation (otherwise ambiguous overload of operator...)
#endif
//namespace fastmath
//{
	...
	class FixedPoint : public FixedPointInfo<precision_bits>
	{
		public:
			...
			explicit FixedPoint( float rhs )			: v( (int32_t)( rhs *  static_cast<float>(this->ONE) + ( rhs < 0 ? -0.5f : 0.5f ) ) ) {}
			explicit FixedPoint( double rhs )			: v( (int32_t)( rhs * static_cast<double>(this->ONE) + ( rhs < 0 ? -0.5f : 0.5f ) ) ) {}
			...
			inline operator float()									{ return v * ( 1.0f / static_cast<float>( this->ONE ) ); }
			inline operator double()								{ return v * ( 1.0f / static_cast<double>( this->ONE ) ); }
			 ...
			 */
			inline uint32_t		getFraction() const { return( v & this->FRACTION_MASK ); }
...
*/
			...
			{
				return FixedPoint::fromRaw( ( v & ( 1 << 31 ) ) ? -static_cast<int32_t>( this->ONE ): static_cast<int32_t>( this->ONE ) );
			}
			...
			inline FixedPoint round()
			{
				...
				if( tmp > 0 ) tmp += static_cast<int32_t>( this->ROUND ); else tmp-= static_cast<int32_t>( this->ROUND );
				tmp /= static_cast<int32_t>( this->ONE );
	...
	class HighPrecision : public FixedPointInfo<bits>
	{
		public:
			...
			inline static int32_t mul( int32_t l, int32_t r )
			{
				...
				t += static_cast<int32_t>( HighPrecision::ROUND );
				t >>= bits;
				return int32_t(t);
	...
	class MidPrecision : public FixedPointInfo<bits>
	{
		public:
			...
			inline static int32_t mul( int32_t l, int32_t r )
			{
				...
				if (a1 < 0) {a1 ^= static_cast<int32_t>( MidPrecision::SIGN_BIT );}
				if (sign) {a1 = -a1;}
				return a1;
			}
			...
			inline static int32_t div( int32_t l, int32_t r )
			{
				...
				if (res < 0) {res ^= static_cast<int32_t>( MidPrecision::SIGN_BIT );}
				if (sign) {res = -res;}
				return res;
			}
